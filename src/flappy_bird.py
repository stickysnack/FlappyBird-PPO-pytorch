...
class FlappyBird(object):
    ...
    image_size = 84
    def __init__(self, name="FlappyBird", device=None, fps=None, render=True):
        self.name = name
        ...
        self.render = render
        # allow caller to override frame rate to speed up training
        self.fps = fps or self.__class__.fps
    ...
    def next_frame(self, action):
        ...
        if self.is_collided():
            terminal = True
            reward = -1
            # preserve device/render/fps settings across resets
            self.__init__(self.name, device=self.device, fps=self.fps, render=self.render)
        ...
        if self.render:
            ...
            # when not rendering, avoid limiting speed
            self.fps_clock.tick(0)
